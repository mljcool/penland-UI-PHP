...
*/
(function() {
    var Loaded = 0;//CHECK IF PAGE LOAD ONCE
    var f = function(e) {
    ...
    f.prototype.setStatus = function(e, f) {
        ...
        if (jQuery.isArray(e))
            for (b = 0; b < e.length; b++)
                e[b].data && e[b].data.number && (a = e[b].data.paging);
        else
     
            e.data && (a = e.data.paging);
        !a || 0 >= a.pagesNumber ? (
                 this.$control.parent().addClass("jplist-empty"),
                 $('.event-filter').addClass("jplist-empty")
            ) : (
                 this.$control.parent().removeClass("jplist-empty"),
                 $('.event-filter').removeClass("jplist-empty"),
                 $(".searchResultCount").text(a.itemsNumber)//ADD RESULT COUNT NUMBER
        )
 
        if( Loaded===1){
            $(".totalResultsCount").text(a.itemsNumber);//ADD TOTAL ITEM NUMBER
        }
    }
    ;
...
(function() {
    ...
    g.prototype.getDeepLink = function() {
        var a = "", b;
        this.inDeepLinking && (b = f(this, !1),
        b.data && (jQuery.isNumeric(b.data.currentPage) && (a = this.name + this.options.delimiter0 + "currentPage=" + b.data.currentPage),
        jQuery.isNumeric(b.data.number) || "all" === b.data.number)) && (a = this.name + this.options.delimiter0 + "number=" + b.data.number);
        return a
    }
    ;
    ...
    g.prototype.setStatus = function(a, b) {
        ...
        if (jQuery.isArray(a))
            for (var d = 0; d < a.length; d++)
                a[d].data && (c = a[d].data.paging);
        else
            a.data && (c = a.data.paging);
...
(function() {
    var f = function(a, b, c, d) {
        ...
        for (var f = b; f < c; f++)
            e += '<li data-type="page" ',
            f === d && (e += ' class="jplist-current active" data-active="true" '),
            b = f + 1,
            e += ' data-number="' + f + '" ',
            e += '><a href="#">' + b + "</a></li>";
    ...
    g.prototype.build = function(a) {
        if (0 <= a.currentPage && a.currentPage < a.pagesNumber) {
            ...
            this.$control.hide();
        0 === a.currentPage ? (this.$jplistFirst.addClass("disabled"),
        this.$jplistPrev.addClass("disabled")) : (this.$jplistFirst.removeClass("disabled"),
        this.$jplistPrev.removeClass("disabled"));
        a.currentPage == a.pagesNumber - 1 ? (this.$jplistNext.addClass("disabled"),
        this.$jplistLast.addClass("disabled")) : (this.$jplistNext.removeClass("disabled"),
        this.$jplistLast.removeClass("disabled"))
    }
    ;
...
(function() {
    var f = function(b) {
      ...
      , g = function(b) {
        b.params.$buttons.on("click", function(a) {
            ...
            a.preventDefault();
            d = e(b, !1);
            if (a = jQuery(this).attr("data-number"))
                d.data.number = a;
    ...
    a.prototype.getDeepLink = function() {
        ...
        a.data && (jQuery.isNumeric(a.data.number) || "all" === a.data.number) && (b = this.name + this.options.delimiter0 + "number=" + a.data.number));
        return b
    }
    ;
    ...
    a.prototype.setStatus = function(a, c) {
        ...
        if (jQuery.isArray(a))
            for (var e = 0; e < a.length; e++)
                a[e].data && (jQuery.isNumeric(a[e].data.number) || "all" === a[e].data.number) && (d = a[e].data.number);
        else
            a.data && (jQuery.isNumeric(a.data.number) || "all" === a.data.number) && (d = a.data.number);
        this.params.$buttons.attr("data-jplist-selected", !1);
        d = 0 === d ? f(this) : this.params.$buttons.filter('[data-number="' + d + '"]');
        0 < d.length && (d.attr("data-jplist-selected", !0),
        this.params.$textBox.text(d.text()))
    }
    ;
